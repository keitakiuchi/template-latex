...
### .codex/config.toml：Codex CLI/IDE の設定

- Codex CLI の既定モデル（gpt-5）、サンドボックス、承認ポリシーを管理
- 応答言語を日本語に固定し、LaTeX ガイドラインとの整合を保つ
- Codex IDE でポリシーを変更する場合はこのファイルを調整する
...
- ビルド中間ファイルは `build/` に出力
- 最終 PDF は `build/pdf/` ディレクトリに生成
...
- 日本語文書には LuaLaTeX と luatexja パッケージを使用（preamble.tex に設定済み）
- コンパイルには lualatex を使用
...
- `utility/check_env.py` : `latexmk` / `lualatex` の有無と必須ディレクトリ構成を確認
- `utility/start.py` : 初期セットアップ用（必要ディレクトリの作成と環境チェック呼び出し）
- `utility/start.sh` : シェルから同様の初期セットアップを実施
- `sharing/ai-driven-coding.py` : `ai-driven-coding.md` を template-latex 用に再生成し、`sharing/ai-driven-coding.md` を更新
- `environment.yml` : `conda env create --file environment.yml --name latex-template` で Codex/Gemini 向け補助環境を構築
- 実験用のビルド検証は `workbench/scripts/test_ai_integration.py` で行い、結果は `workbench/results/` に保存
